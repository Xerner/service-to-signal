...
use up_rust::{communication::InMemoryRpcServer, LocalUriProvider, StaticUriProvider};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!("Listening for incoming horn RPC requests");
...
async fn default_setup(
    ...
    let transport = get_zenoh_transport(uri_provider.clone(), args.get_zenoh_config()?).await;
